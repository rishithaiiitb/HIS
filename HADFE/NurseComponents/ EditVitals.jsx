...
export default function EditVitals({ navigation, route }) {
  ...
  const fetchOldVitals = async () => {
    try {
      const token = await AsyncStorage.getItem("token");
      const response = await axios.get(
        `${API_BASE_URL}/nurse/viewVitalsById/${patientId}/${vitalid}`,
        {
          headers: {
      ...
      const oldVitals = response.data;
      // Split the combined blood pressure value into systolic and diastolic values
      const [systolic, diastolic] = oldVitals.bp.split("/");
      setVitals({ ...oldVitals, systolic, diastolic });
    } catch (error) {
      if (error.response && error.response.status === 500) {
        ...
        console.error("Error fetching old vitals:", error);
      }
    }
  };
  ...
  const handleSubmit = async () => {
    try {
      ...
      const pulseRegex = /^(200|1\d{2}|[2-9]\d)(\.\d)?$/; // Matches numbers between 20 and 200
      const temperatureRegex = /^([9]\d|[1][0-0][0-8])(\.\d)?$/; // Matches numbers between 90 and 108

      if (vitals.weight !== "" && weightRegex.test(vitals.weight)) {
        validVitals.weight = vitals.weight;
      }

      if (vitals.height !== "" && heightRegex.test(vitals.height)) {
        validVitals.height = vitals.height;
      }

      if (vitals.pulse !== "" && pulseRegex.test(vitals.pulse)) {
        validVitals.pulse = vitals.pulse;
      }

      if (
        vitals.spo2 !== "" &&
        spo2Regex.test(vitals.spo2) &&
        vitals.spo2 <= 100
      ) {
        validVitals.spo2 = vitals.spo2;
      }

      if (vitals.systolic !== "" && systolicRegex.test(vitals.systolic)) {
        validVitals.systolic = vitals.systolic;
      }

      if (vitals.diastolic !== "" && diastolicRegex.test(vitals.diastolic)) {
        validVitals.diastolic = vitals.diastolic;
      }

      if (
        vitals.temperature !== "" &&
        temperatureRegex.test(vitals.temperature)
      ) {
        validVitals.temperature = vitals.temperature;
      }

      if (Object.keys(validVitals).length === 0) {
        console.log("No valid fields to submit.");
        return;
      }
      // Combine systolic and diastolic values into blood pressure format
      const bpValue = `${validVitals.systolic}/${validVitals.diastolic}`;
      const token = await AsyncStorage.getItem("token");

      const response = await axios.put(
        `${API_BASE_URL}/nurse/editVitals/${vitalid}`,
        {
      ...
      console.log("Vitals edited successfully:", response.data);
      Alert.alert("Vitals edited successfully");
      const [systolic, diastolic] = response.data.bp.split("/");
      // After successful submission, update the input fields with new vitals
      setVitals({
        temperature: response.data.temperature || "", // Use response data
      ...
      <ImageBackground source={BG_Vitals} style={styles.content}>
        ...
        <ScrollView contentContainerStyle={styles.formContainer}>
          <View style={styles.formcontent}>
            <Text style={styles.heading}>Vital Signs Modification</Text>
                    ...
                    <View style={styles.bpInputWrapper}>
                      ...
                      <View style={styles.bpinputWithIcon}>
                        ...
                        <TextInput
                          ...
                          value={vitals.diastolic.toString()}
