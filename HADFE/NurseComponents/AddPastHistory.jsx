...
export default function AddPastHistory({ navigation, route }) {
  ...
  const handleSubmit = async () => {
    try {
      ...
      const response = await axios.post(
        `${API_BASE_URL}/nurse/addPastHistory/${patientId}`,
        {
          ...pastHistory,
        },
        {
      ...
      console.log("Past history added successfully:", response.data);
      Alert.alert("Past History added successfully");
      setPastHistory({
        disease: "",
        dosage: "",
        medicine: "",
  ...
  const validateInputs = () => {
    ...
    if (!textRegex.test(pastHistory.disease.trim())) {
      showAlert("Invalid input", "Please enter a valid disease name");
      return false;
    }

    if (!textRegex.test(pastHistory.dosage.trim())) {
      showAlert("Invalid input", "Please enter a valid dosage");
      return false;
    }

    if (!textRegex.test(pastHistory.medicine.trim())) {
      showAlert("Invalid input", "Please enter a valid medicine name");
      return false;
    }

    if (!textRegex.test(pastHistory.remarks.trim())) {
        ...
        <ScrollView contentContainerStyle={styles.formContainer}>
          <View style={styles.formcontent}>
            <Text style={styles.heading}>
              Patient's Health Journey: Add Past Medical History
            </Text>
            <View style={styles.inputRow}>
              <View style={styles.inputColumn}>
                ...
                {pastHistory.disease !== "" &&
                  !/^[a-zA-Z\s]+$/.test(pastHistory.disease) &&
                  renderErrorMessage(
                    "Disease Name must contain only alphabets and spaces."
                  )}
                <View style={styles.inputContainer}>
                ...
                {pastHistory.medicine !== "" &&
                  !/^[a-zA-Z0-9\s]+$/.test(pastHistory.medicine) &&
                  renderErrorMessage(
                    "Medicine Name must contain only alphabets,numbers and spaces."
                  )}
              </View>
              <View style={styles.inputColumn}>
                <View style={styles.inputContainer}>
                ...
                {pastHistory.dosage !== "" &&
                  !/^\d+\s(mg|mcg)$/.test(pastHistory.dosage) &&
                  renderErrorMessage(
                    "Dosage should be in the format of e.g: 20 mg/mcg"
                  )}
                <View style={styles.inputContainer}>
                  ...
                  >
                    <Text>{pastHistory.recordedAt.toDateString()}</Text>
            ...
            {pastHistory.remarks !== "" &&
              !/^[a-zA-Z\s]+$/.test(pastHistory.remarks) &&
              renderErrorMessage(
                "Remarks must contain only alphabets and spaces."
              )}
            <TouchableOpacity
